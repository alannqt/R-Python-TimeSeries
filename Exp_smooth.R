...
#step 3A: from the decompose plot we observed some seasonality, hence we will be using Holt-winter exponential.
myts_hw12 <- HoltWinters(myts_training_12)
myts_hw12

#Step 4A: plot to see the fit of the model
plot(myts_hw12)
#Step 5A: forcast for next 12 months
myts_hw12_forecast <- forecast(myts_hw12, h=12)
#Step 6A: plot the forecast
plot(myts_hw12_forecast)
#step 7A: check RMSE
error <- myts_hw12_forecast$mean - myts_test_12
RMSE_hw12 <- sqrt(mean(error^2))

#step 7A: check RMSE for 6 months
error_manual <- myts_hw12_forecast$mean[1:6] - myts_test_12[1:6]
RMSE_hw12_manual <- sqrt(mean(error_manual^2))
